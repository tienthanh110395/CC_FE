...
def bootstrap_roll_back(){
    ...
    try {
        ...
        //Credential of user
       withCredentials([usernamePassword(credentialsId: 'vdtc-gitlab', usernameVariable: 'username',
    ...
    if (pushCommitInOpenMR(env.gitlabBranch)) {
        ...
        try{
            ...
            if (currentBuild.result == "SUCCESS") {
                ...
                buildResultStr = "Build Success. Deploy version ${env.project_version} result. Environment IP: <b><a href='http://${env.STAGING_IP}:${env.STAGING_PORT}'>http://${env.STAGING_IP}:${env.STAGING_PORT}</a></b> "
...
def bootstrapMergeRequestBuild() {
    try {
        ...
        if (env.sourceBranchCommitID != env.targetBranchCommitID) {
            ...
            if (currentBuild.result == "SUCCESS") {
                buildIcon = ":white_check_mark:"
                buildResultStr += "Build Success. Deploy version ${env.project_version} result. Environment IP: <b><a href='http://${env.STAGING_IP}'>${env.STAGING_IP}</a>http://${env.STAGING_IP}:${env.STAGING_PORT}</b>"
            } else if (currentBuild.result == "ABORTED") {
                buildIcon = ":warning:"
                buildResultStr = "Build Canceled."
            } else if (currentBuild.result == "FAILURE") {
...
def bootstrapDeployToProduction() {
    ...
    try {
        loadCIConfigFile()
        println "Result : " + checkBuildResult("${env.buildUrlCI}")
        if(checkBuildResult("${env.buildUrlCI}") == 'SUCCESS'){
            jenkinsfile_CD.deployToProduction()
            env.DEPLOY_RESULT_DESCRIPTION += "<h4>:white_check_mark: Deploy Final Result: Success.<h4>"
        }else {
            error 'Job Build CI staging failded!'
            throw err
        }
    } catch (FlowInterruptedException interruptEx) {
        echo "${interruptEx}"
...
def bootstrapDeployToProductionVTNet() {
    env.DEPLOY_RESULT_DESCRIPTION = ""
    try {
        loadCIConfigFile()
        jenkinsfile_CD.deployToProductionVTNet()
        env.DEPLOY_RESULT_DESCRIPTION += "<h4>:white_check_mark: Deploy Final Result: Success.<h4>"
    } catch (FlowInterruptedException interruptEx) {
        echo "${interruptEx}"
        currentBuild.result = "ABORTED"
...
def bootstrapDeployToPreRelease() {
    env.DEPLOY_RESULT_DESCRIPTION = ""
    try {
        loadCIConfigFile()
        jenkinsfile_CD.deployToPreRelease()
        env.DEPLOY_RESULT_DESCRIPTION += "<h4>:white_check_mark: Deploy Final Result: Success.<h4>"
    } catch (FlowInterruptedException interruptEx) {
        echo "${interruptEx}"
        currentBuild.result = "ABORTED"
...
def checkBuildResult(buildURL) {
	withCredentials([usernamePassword(credentialsId: 'jenkins_api_token_new', usernameVariable: 'usernamejenkins', passwordVariable: 'token')]) {
			...
			return jenkinsfile_utils.jsonParse(buildInfoResp.content)["result"]
		}
}
/**
...
return [
    ...
    bootstrapDeployToProductionVTNet: this.&bootstrapDeployToProductionVTNet
]

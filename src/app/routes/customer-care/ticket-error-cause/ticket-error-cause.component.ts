...
import { AddEditTicketErrorCauseComponent } from './add-edit-ticket-error-cause/add-edit-ticket-error-cause.component';
import { ViewTicketErrorCauseComponent } from './view-ticket-error-cause/view-ticket-error-cause.component';
...
export class TicketErrorCauseComponent extends BaseComponent implements OnInit {
  formTicketErrorCause: FormGroup;
  check = [];
  constructor(
  ...
  buildForm() {
    this.formTicketErrorCause = this.fb.group({
      name: [''],
      code: [''],
      status: [''],
      startrecord: [0],
  ...
  buildColumns() {
    this.columns = [
      ...
      { i18n: 'ticket-error-cause.ticketErrorCauseNameLv1', field: 'name', width: '100px' },
      { i18n: 'ticket-error-cause.ticketErrorCauseIdLv1', field: 'code', width: '20px' },
      { i18n: 'ticket-error-cause.ticketErrorCauseNameLv2', field: 'name2', width: '100px' },
      { i18n: 'ticket-error-cause.ticketErrorCauseIdLv2', field: 'code2', width: '20px' },
      { i18n: 'ticket-error-cause.ticketErrorCauseNameLv3', field: 'name3', width: '100px' },
      { i18n: 'ticket-error-cause.ticketErrorCauseIdLv3', field: 'code3', width: '20px' },
      { i18n: 'ticket-error-cause.description', field: 'description', width: '100px' },
      { i18n: 'ticket-error-cause.status', field: 'statusName', width: '50px' },
    ];
  }

  onPageChange(event) {
    this.pageIndex = event.pageIndex;
    this.formTicketErrorCause.controls.startrecord.setValue(
      event.pageIndex === 0 ? event.pageIndex : event.pageIndex * event.pageSize
    );
    this.formTicketErrorCause.controls.pagesize.setValue(event.pageSize);
    this.getDataTicketErrorCause();
  }

  onSearch() {
    this.pageIndex = 0;
    this.formTicketErrorCause.controls.startrecord.setValue(0);
    this.getDataTicketErrorCause();
  }
  ...
  getDataTicketErrorCause() {
    if (this.formTicketErrorCause.valid) {
      const searchObj = Object.assign({}, this.formTicketErrorCause.value);
      this.setValueToObject(searchObj);
      if (this.formTicketErrorCause.value.startrecord == 0) {
        searchObj.startrecord = 0;
      }
      this._ticketService.searchTicketErrorCause(searchObj).subscribe(rs => {
  ...
  deleteTicketErrorCause(row) {
    ...
    dialogRef.afterClosed().subscribe(dialogResult => {
      if (dialogResult) {
        this._ticketService.deleteTicketErrorCause(row.ticketErrorCauseId).subscribe(res => {

...
@Component({
  selector: 'app-ticket-edit-history',
  templateUrl: './ticket-edit-history.component.html',
  styleUrls: ['./ticket-edit-history.component.scss']
})
export class TicketEditHistoryComponent extends BaseComponent implements OnInit {
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<TemplateRef<TicketEditHistoryComponent>>,
  ...
  async getListStageAndStation() {
    let no = 1;
    const res = (await this._sharedDirectoryService.getStageAndStation().toPromise());
    if (res.mess.code === HTTP_CODE.SUCCESS) {
      this.stationList = res.data.listData.map(val => {
        return {
          no: no++,
  ...
  getTicketSla(ticketTypeId, sourceId, priorityId) {
    if (priorityId) {
      this._commonCCService.getTicketSLA({ siteId: AppStorage.get('user-info')?.siteId || null, ticketTypeId, sourceId, priorityId }).subscribe(res => {
        if (res.mess.code == HTTP_CODE.SUCCESS) {
          this.processTicketForm.get('slaDate').setValue(res.data.slaDate);
        } else {
  ...
  getValueTicket() {
    this._ticketService.getTicketById(this.dataDialog.ticketId).subscribe(res => {
      ...
      this.processTicketForm.controls.custTypeId.setValue(Number(res.data.custTypeId) ? Number(res.data.custTypeId) : null);//Loại khách hàng
      ...
      this.processTicketForm.controls.custName.setValue(res.data.custName ? res.data.custName : null);//Người phản ánh
      this.processTicketForm.controls.email.setValue(res.data.email ? res.data.email : null);//Email
      this.processTicketForm.controls.custAddress.setValue(res.data.custAddress ? res.data.custAddress : null);//Địa chỉ
      this.processTicketForm.controls.priorityId.setValue(Number(res.data.priorityId) ? Number(res.data.priorityId) : null);//Mức độ ưu tiên
      this.processTicketForm.controls.sourceId.setValue(Number(res.data.sourceId) ? Number(res.data.sourceId) : null);//Hình thức tiếp nhận
      ...
      this.processTicketForm.controls.location.setValue(res.data.custAddress ? res.data.custAddress : null);//Địa chỉ chi tiết
      this.processTicketForm.controls.province.setValue(res.data.provinceName ? res.data.provinceName : null);//Tỉnh/thành phố
      this.processTicketForm.controls.district.setValue(res.data.districtName ? res.data.districtName : null);//Quận/huyện
      this.processTicketForm.controls.commune.setValue(res.data.communeName ? res.data.communeName : null);//Xã/phường
      if (res.data.stationId || res.data.stageId) {
        if (res.data.stationId) {
          this.processTicketForm.controls.stationId.setValue(res.data.stationId ? res.data.stationId : null);//Trạm/đoạn
        }
        else if (res.data.stageId) {
          this.processTicketForm.controls.stationId.setValue(res.data.stageId ? res.data.stageId : null);//Trạm/đoạn
        }
      }
      this.processTicketForm.controls.ticketKind.setValue(Number(res.data.ticketKind) ? Number(res.data.ticketKind) : null);//Loại lỗi
      if (res.data.requestDate == null) {
        this.processTicketForm.controls.requestDate.valid;
      } else {
        this.processTicketForm.controls.requestDate.setValue(moment(res.data.requestDate, COMMOM_CONFIG.DATE_TIME_FORMAT).toDate() ? moment(res.data.requestDate, COMMOM_CONFIG.DATE_TIME_FORMAT).toDate() : null);//Ngày hẹn khách hàng
      }
      this.processTicketForm.controls.status.setValue(Number(res.data.status) ? Number(res.data.status) : null);//Trạng thái phản ánh
      this.processTicketForm.controls.slaDate.setValue(res.data.slaDate ? res.data.slaDate : null);//Hạn xử lý theo KPI
      ...
      this.processTicketForm.controls.supportInfo.setValue(res.data.supportInfo ? res.data.supportInfo : null);//Thông tin bổ sung
      this.processTicketForm.controls.note.setValue(res.data.note ? res.data.note : null);//Ghi chú
      ...
      this.listFile.data = res.data.ticketAttachmentEntityList;
    })
  }
  ...
  onSubmit() {
    ...
    object.ticketId = this.dataDialog.ticketId;
    object.custTypeId = Number(object.custTypeId) ? Number(object.custTypeId) : null;
    object.priorityId = Number(object.priorityId) ? Number(object.priorityId) : null;
    object.sourceId = Number(object.sourceId) ? Number(object.sourceId) : null;
    ...
    object.provinceName = this.processTicketForm.value.province ? this.processTicketForm.value.province.label : '';
    object.districtName = this.processTicketForm.value.district ? this.processTicketForm.value.district.label : '';
    object.communeName = this.processTicketForm.value.commune ? this.processTicketForm.value.commune.label : '';
    if (object.provinceName == undefined || object.districtName == undefined || object.communeName == undefined) {
      object.provinceName = object.province || null;
      object.districtName = object.district || null;
      object.communeName = object.commune || null;
    }
    delete object.commune;
    delete object.district;
    delete object.province;

    object.status = Number(object.status) ? Number(object.status) : null;
    object.ticketKind = Number(object.ticketKind) ? Number(object.ticketKind) : null;

...
import { HTTP_CODE, CC_PRIORITY, SharedDirectoryService, CC_TICKET_STATUS, ACTION_TYPE } from '@app/shared';
...
import { TicketExtentComponent } from './ticket-extent/ticket-extent.component';
import { saveAs } from 'file-saver'

@Component({
  selector: 'app-search-cc',
  templateUrl: './search-cc.component.html',
  styleUrls: ['./search-cc.component.scss']
})
export class SearchCcComponent extends BaseComponent implements OnInit {
  ...
  constructor(
    ...
    super(_sharedDirectoryService, RESOURCE.CC_CUSTOMER, _toastrService, _translateService);
  ...
  buildColumn() {
    this.columns = [
      ...
      { i18n: 'common.action', field: 'action', width: '90px', sticky: true, type: 'actionCustom' },
      { i18n: 'customer-care.search-cc.ticketCode', field: 'ticketId', width: '40px' }, //Mã phản ánh
      { i18n: 'customer-care.search-cc.plateNumber', field: 'plateNumber', width: '50px' }, //Biển số xe
      { i18n: 'customer-care.search-cc.contractNo', field: 'contractNo', width: '50px' }, //Số hợp đồng
      { i18n: 'customer-care.search-cc.phoneNumber', field: 'phoneNumber', width: '50px' }, //Số điện thoại theo hợp đồng
  ...
  setValueToObject(searchObj: any) {
    ...
    searchObj.phoneNumber = searchObj.phoneNumber ? searchObj.phoneNumber.trim() : null;
    searchObj.createUser = searchObj.createUser ? searchObj.createUser.trim() : null;
    searchObj.staffName = searchObj.staffName ? searchObj.staffName.trim() : null;
  ...
  openReceiveTicket(item) {
    this._router.navigate(['customer-care', 'ticket-process'], {
      queryParams: {
        ticketId: item.ticketId, expireStatus: item.expireStatus, contractNo: item.contractNo, plateNumber: item.plateNumber, siteId: item.siteId,
        status: item.status, l1TicketTypeId: item.l1TicketTypeId, l2TicketTypeId: item.l2TicketTypeId, ticketErrorCauseIdL1: item.ticketErrorCauseIdL1,
        ticketErrorCauseIdL2: item.ticketErrorCauseIdL2, ticketSiteIdL1: item.ticketSiteIdL1, ticketSiteIdL2: item.ticketSiteIdL2
      }
    });
  }

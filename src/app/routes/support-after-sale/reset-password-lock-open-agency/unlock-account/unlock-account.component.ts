...
export class UnlockAccountComponent extends BaseComponent implements OnInit {
  ...
  constructor(
    ...
    public dialogRef?: MatDialogRef<TemplateRef<UnlockAccountComponent>>,
  ) {
    super();
  ...
  getListDocumentTypeObject() {
    this._resetPasssLockOpenAgencyService.getReason(ACTION_TYPE.KH_YEUCAU_UNLOCKACC).subscribe(res => {
  ...
  lockAccCTV() {
    ...
    dialogRef.afterClosed().subscribe(dialogResult => {
      if (dialogResult) {
        this._resetPasssLockOpenAgencyService.lockAccountCTV(this.userId, this.isLock, this.listDataProfile[0] ? this.listDataProfile[0].fileBase64 : '').subscribe(res => {
          if (res.mess.code === HTTP_CODE.SUCCESS) {
            this._toastr.success(this._translate.instant('reset-agency.lock-accCTV-success'));

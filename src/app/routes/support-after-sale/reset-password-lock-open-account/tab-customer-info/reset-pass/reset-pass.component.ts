...
import { ACTION_REASON, HTTP_CODE } from '@app/shared';
...
export class ResetPassComponent extends BaseComponent implements OnInit {
  ...
  constructor(
    ...
    public dialogRef?: MatDialogRef<TemplateRef<ResetPassComponent>>,
  ) {
    super();
  ...
  ngOnInit() {
    ...
    this.header = this._translate.instant('common.button.reset');
  ...
  resetPassword() {
    ...
    dialogRef.afterClosed().subscribe(dialogResult => {
      if (dialogResult) {
        this._resetPasswordLockOpenAccountService.resetPassword(this.accountUserId, "", this.listDataProfile[0] ? this.listDataProfile[0].fileBase64 : '').subscribe(res => {
          if (res.mess.code === HTTP_CODE.SUCCESS) {
            this._toastr.success(this._translate.instant('reset-agency.reset-passCPT-success'));

...
@Component({
  selector: 'app-import-file-processing-time-dialog',
  templateUrl: './import-file-processing-time-dialog.component.html',
  styleUrls: ['./import-file-processing-time-dialog.component.scss']
})
export class ImportFileProcessingTimeDialogComponent extends BaseComponent implements OnInit {
  ...
  buildForm() {
    this.form = this.fb.group({
      inputFile: '', //Filter Nguyên nhân cấp 1
  ...
  uploadFile() {
    ...
    this.ticketSlaService.importFileTicketTime(data).subscribe(res => {
  ...
  downloadTemplateTicketTime() {
    this.ticketSlaService.downloadTemplateTicketTime()
      .subscribe(res => {
        const contentDisposition = res.headers.get('content-disposition');
        const filename = contentDisposition.split(';')[1].split('filename')[1].split('=')[1].trim();
        saveAs(res.body, filename);
